...
func createParamAuthSession(apiID string) *user.SessionState {
	session := new(user.SessionState)
	session.Rate = 10000
	session.Allowance = session.Rate
	session.LastCheck = time.Now().Unix()
	...
	session.QuotaRemaining = 10
	session.QuotaMax = -1
	session.AccessRights = map[string]user.AccessDefinition{apiID: {APIName: "Tyk Test API", APIID: apiID, Versions: []string{"default"}}}
	return session
}
